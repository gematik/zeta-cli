...
- Konnektor SOAP faults now show what the Konnektor actually said. The fault content was being dropped on decode — SOAP 1.1 sends `faultcode` / `faultstring` / `detail` unqualified while the generated envelopes expect them in the `SOAP-ENV` namespace, so every fault came back empty and could only be reported as `ExternalAuthenticate(cardHandle=…) reported a SOAP fault`. Faults are now read off the raw response, gematik's `<detail><Error><Trace>` included, so the message carries the reason and the error code: `… reported a SOAP fault: Technical Error (code 4018: Der Aufruf ist nicht zulässig)`
